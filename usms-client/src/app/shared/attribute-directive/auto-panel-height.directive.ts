 ...
 * 用法：<div class="parentClassName"><div appAutoPanelHeight="parentClassName"></div></div>
 * 备注：需设置datatable的scrollable属性
 * @Author Warren Chan 2017年6月28日 11:14:00
 */
@Directive({
  selector: '[appAutoPanelHeight]'
})
export class AppAutoPanelHeightDirective implements AfterViewInit, AfterViewChecked {

  @Input('appAutoPanelHeight') parentClassName: string; // 指定父节点类名

  ele: Element; // 当前元素
  pEle: Element; // 父元素
  panelEle: Element; // 需要动态设置滚动条的元素
  deduct = 103; // 扣除高度（一般扣除分页条的高度，看需求自己写成输入属性）
